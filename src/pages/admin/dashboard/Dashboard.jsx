...
function Dashboard() {
  ...
  const { mode, product, users } = context

  const totalOrder=JSON.parse(localStorage.getItem('orderedItem'))
          ...
          <div className="flex flex-wrap -m-4 text-center" data-aos="zoom-in" data-aos-duration="1000">
            ...
            <div className="p-4 md:w-1/4 sm:w-1/2 w-full">
              <div className=" border-2 hover:shadow-purple-600 shadow-[inset_0_0_10px_rgba(0,0,0,0.6)] bg-gray-100 border-gray-300    px-4 py-3 rounded-xl" style={{ backgroundColor: mode === 'dark' ? 'rgb(46 49 55)' : '', color: mode === 'dark' ? 'white' : '', }}>
                ...
                <h2 className="title-font font-medium text-3xl text-black fonts1" style={{ color: mode === 'dark' ? 'white' : '' }}>{totalOrder.length}</h2>
